...
async function renderAlgorithms(area, algorithms, spotlightCards = []) {
  ...
  const CATEGORY_LABELS = { statistici: 'Statistici', neurali: 'Neurali', ibridi: 'Ibridi', generativi: 'G-Family', algoritmo: 'Algoritmo Family', genetico: 'Genetico Family' };
  const CATEGORY_ORDER = ['statistici', 'neurali', 'ibridi', 'generativi', 'algoritmo', 'genetico'];
...
function scrollToGroupHash() {
  ...
  const rawTargetId = parts[0];
  const targetKey = resolveGroupTargetKey(rawTargetId);
  ...
  activateTabByGroupKey(targetKey);
...
function resolveGroupTargetKey(rawGroupId) {
  const cleaned = String(rawGroupId || '')
    .trim()
    .toLowerCase()
    .replace(/^group-/, '');

  if (['statistici', 'statistico', 'statistica', 'statistiche', 'logici', 'logico'].includes(cleaned)) {
    return 'statistici';
  }
  if (['neurali', 'neurale', 'neural'].includes(cleaned)) {
    return 'neurali';
  }
...
function activateTabByGroupKey(groupKey) {
  const normalized = resolveGroupTargetKey(`group-${groupKey}`);
  const tabsRoot = document.querySelector('[data-tabs-root]');
  if (!tabsRoot) return;
  const targetBtn = tabsRoot.querySelector(`[data-tab-target="${normalized}"]`);
