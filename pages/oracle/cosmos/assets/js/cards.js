const CARDS = {
  ...
  makeOrbitCardTexture(payload = {}) {
    const md = payload.md;
    const baseRgb = payload.baseRgb;
    const deps = payload.deps || {};
    const texEngine = window.SECC_CARD_TEXTURE;
    if (!texEngine || typeof texEngine.makeCardTex !== 'function') return null;
    try {
      return texEngine.makeCardTex(md, baseRgb, deps);
    } catch (_error) {
      return null;
    }
  }
