...
const elements = {
  ...
  latestContest: document.querySelector('[data-latest-draw-contest]'),
  latestDate: document.querySelector('[data-latest-draw-date]'),
  latestNumbers: document.querySelector('[data-latest-draw-numbers]'),
...
function showUnavailable(message) {
  ...
  if (elements.latestContest) elements.latestContest.textContent = 'n/d';
  if (elements.latestDate) elements.latestDate.textContent = 'n/d';
  if (elements.latestNumbers) elements.latestNumbers.textContent = 'n/d';
...
function updateSummary() {
  if (!rows.length) {
    ...
    if (elements.latestContest) elements.latestContest.textContent = '-';
    if (elements.latestDate) elements.latestDate.textContent = '-';
    if (elements.latestNumbers) elements.latestNumbers.textContent = '-';
    return;
  }
...
function updateLatestDrawBridge() {
  if (!rows.length) return;
  const latest = rows[0] || [];
  const contestIndex = headers.findIndex((header) => {
    const normalized = String(header || '').trim().toLowerCase();
    return normalized.includes('concorso') || normalized.includes('sequenziale');
  });
  const sixNumberIndexes = headers.reduce((acc, header, index) => {
    const normalized = String(header || '').trim().toLowerCase();
    if (/^n[1-6]$/.test(normalized)) acc.push(index);
    return acc;
  }, []);
  const contest = contestIndex >= 0 ? latest[contestIndex] : '-';
  const date = dateIndex >= 0 ? latest[dateIndex] : '-';
  const numbers = sixNumberIndexes
    .map((index) => String(latest[index] || '').trim())
    .filter(Boolean)
    .join(' ');
  const bridge = document.querySelector("[data-bridge-id='archive_bridge_frequency']");
  if (bridge) {
    bridge.innerHTML = `
      <p><strong>Ultimo concorso:</strong> <span data-latest-draw-contest>${escapeHtml(contest || '-')}</span> · <strong>Data:</strong> <span data-latest-draw-date>${escapeHtml(date || '-')}</span></p>
      <p><strong>Sestina estratta:</strong> <span data-latest-draw-numbers>${escapeHtml(numbers || '-')}</span></p>
    `;
  }
  if (elements.latestContest) elements.latestContest.textContent = contest || '-';
  if (elements.latestDate) elements.latestDate.textContent = date || '-';
  if (elements.latestNumbers) elements.latestNumbers.textContent = numbers || '-';
}
